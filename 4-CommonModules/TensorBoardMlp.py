...
def train():
    ...
    # 实例化TensorBoard
    if os.path.exists("./4-CommonModules/tensorboard"):
        shutil.rmtree("./4-CommonModules/tensorboard")
    summary_writer = tf.summary.create_file_writer(
        "./4-CommonModules/tensorboard")
    ...
    for batch_index in range(1, num_batches+1):
        ...
        with summary_writer.as_default():  # 选择记录器
            tf.summary.scalar("loss", loss, step=batch_index)  # 保存要可视化的变量

    with summary_writer.as_default():
        tf.summary.trace_export(name="model_trace", step=0,
                                profiler_outdir="./4-CommonModules/tensorboard")
